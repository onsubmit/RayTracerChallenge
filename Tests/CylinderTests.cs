namespace Tests
{
    ...
    public class CylinderTests
    {
        ...
        public void RayMissesCylinder()
        {
            ...
            Ray ray = new Ray(Tuple4D.CreatePoint(1, 0, 0), Tuple4D.CreateVector(0, 1, 0).Normalize());
            Intersections intersections = cylinder.GetIntersectionsWithImpl(ray);
            Assert.AreEqual(0, intersections.Count);

            ray = new Ray(Tuple4D.CreatePoint(0, 0, 0), Tuple4D.CreateVector(0, 1, 0).Normalize());
        ...
        {
            Cylinder cylinder = new Cylinder();
            Ray ray = new Ray(Tuple4D.CreatePoint(1, 0, -5), Tuple4D.CreateVector(0, 0, 1).Normalize());
            Intersections intersections = cylinder.GetIntersectionsWithImpl(ray);
            ...
            ray = new Ray(Tuple4D.CreatePoint(0.5, 0, -5), Tuple4D.CreateVector(0.1, 1, 1).Normalize());
            intersections = cylinder.GetIntersectionsWithImpl(ray);
            Assert.AreEqual(2, intersections.Count);
            Assert.IsTrue(intersections[0].T.Compare(6.80798));
            Assert.IsTrue(intersections[1].T.Compare(7.08872));
        }
        ...
        {
            Cylinder cylinder = new Cylinder();
            Tuple4D normal = cylinder.GetNormalAtPointImpl(Tuple4D.CreatePoint(1, 0, 0));
            Assert.AreEqual(Tuple4D.CreateVector(1, 0, 0), normal);

            normal = cylinder.GetNormalAtPointImpl(Tuple4D.CreatePoint(0, 5, -1));
            Assert.AreEqual(Tuple4D.CreateVector(0, 0, -1), normal);

            normal = cylinder.GetNormalAtPointImpl(Tuple4D.CreatePoint(0, -2, 1));
            Assert.AreEqual(Tuple4D.CreateVector(0, 0, 1), normal);

            normal = cylinder.GetNormalAtPointImpl(Tuple4D.CreatePoint(-1, 1, 0));
            Assert.AreEqual(Tuple4D.CreateVector(-1, 0, 0), normal);
        }
