...
class Chapter05 {
    constructor() {
        this.run = () => {
            ...
            sphere.transformation = Matrix_1.default.getScalingMatrix(1, 0.75, 1).rotateZ(Constants_1.default.pi_4);
            for (let y = 0; y < canvasSize; y++) {
                const worldY = half - pixelSize * y;
                for (let x = 0; x < canvasSize; x++) {
                    const worldX = -half + pixelSize * x;
                    const p = new Point_1.default(worldX, worldY, wallZ);
                    const ray = new Ray_1.default(rayOrigin, Vector_1.default.fromNumberTuple(p.subtract(rayOrigin)).normalize());
